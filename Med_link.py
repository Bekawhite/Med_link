        ...
        with st.form("referral_form"):
            ...
            with col2:
                referring_hospital = st.selectbox("Referring Hospital*", 
                    ["Jaramogi Oginga Odinga Teaching & Referral Hospital (JOOTRH)",
                     "Kisumu County Referral Hospital", "Lumumba Sub-County Hospital"])
                receiving_hospital = st.selectbox("Receiving Hospital*", 
                    ["Jaramogi Oginga Odinga Teaching & Referral Hospital (JOOTRH)",
                     "Kisumu County Referral Hospital", "Lumumba Sub-County Hospital"])
...
class DriverUI:
        ...
        if ambulance.current_patient and ambulance.status == 'On Transfer':
            ...
            with col2:
                if st.button("🚦 Start Trip", use_container_width=True):
                    self.db.update_ambulance_status(ambulance.ambulance_id, 'On Transfer')
    ...
    def show_vitals_form(self, patient):
        with st.form("vitals_form"):
            ...
            if st.form_submit_button("Update Vitals"):
                patient.vital_signs = {
                    'blood_pressure': bp, 'heart_rate': heart_rate, 'oxygen_saturation': spo2,
    ...
    def complete_mission(self, ambulance, patient):
        ...
        self.notification_service.send_notification(
            ...
            f"Patient {patient.name} has arrived via ambulance {ambulance.ambulance_id}",
            'arrival'
        )
    ...
    def quick_actions(self, ambulance):
        ...
        with col3:
            if st.button("🔧 Maintenance", use_container_width=True):
                ...
                st.rerun()
        if st.button("🚨 EMERGENCY ASSISTANCE", use_container_width=True, type="secondary"):
            st.error("EMERGENCY SIGNAL SENT! Help is on the way.")
            self.notification_service.send_notification(
                "Control Center",
                f"EMERGENCY: Ambulance {ambulance.ambulance_id} requires assistance!",
                'emergency'
            )
